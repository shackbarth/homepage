(function () {
  ...
  var stylesheets = fs.readdirSync(path.join(__dirname, "../public/stylesheets"));
  var loadContentFile = function (filename, done) {
    ...
    fs.readFile(path.join(__dirname, "../content", filename), "utf8", function (err, data) {
      ...
      var blurName = _.find(imageFiles, function (imageFile) {
        return path.basename(imageFile, path.extname(imageFile)) === basename + "-blur";
      });
      var blurPath = blurName ?
        "../images/" + blurName :
        imagePath;
      var stylesheetName = _.find(stylesheets, function (stylesheet) {
        return path.basename(stylesheet) === basename + ".styl";
      });
